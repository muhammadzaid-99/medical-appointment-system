...
async function postSignUp(req, res) {
	...
	try {
		const result = await db.query(
			`INSERT INTO Users (name, email, password) values ('${data.name}', '${data.email}', '${hashed_password}');`
		);

		if (data.isDoctor) {
			const user = await db.query(`SELECT * FROM Users WHERE email='${data.email}'`);
			db.query(
				`INSERT INTO Doctors (doctor_id, department, address, fee) values (${user.rows.at(0).user_id}, '${
					data.department
				}', '${data.address}', ${data.fee});`
			).then((results) => {});
		}
		res.status(200).json({ message: `${data.name} signed up!`, success: true });
	} catch (error) {
